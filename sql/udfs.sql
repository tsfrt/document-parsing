...
--     parse_doc_granite, parse_doc_nougat) that wraps ai_query against the
--     matching endpoint.
...
CREATE OR REPLACE FUNCTION parse_doc_nougat(content BINARY)
RETURNS STRING
COMMENT 'Nougat-base (facebook/nougat-base). Meta encoder-decoder for academic / scientific PDFs (LaTeX-aware).'
RETURN COALESCE(
  ai_query(
    'doc-parser-nougat',
    named_struct('image_b64', base64(content)),
...
RETURNS STRING
COMMENT 'Dispatch document parsing to one of the four OCR endpoints. model in {florence, phi3, granite, nougat}.'
RETURN CASE lower(coalesce(model, 'florence'))
  ...
  WHEN 'nougat'   THEN parse_doc_nougat(content)
