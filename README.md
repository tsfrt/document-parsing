...
| `doc-parser-nougat` | Wraps [`facebook/nougat-base`](https://huggingface.co/facebook/nougat-base) (350M). Meta encoder-decoder for academic / scientific PDFs (LaTeX-aware). Loaded in bf16 (~1 GB). |
| SQL UDFs | `parse_doc_florence`, `parse_doc_phi3`, `parse_doc_granite`, `parse_doc_nougat`, plus a router `parse_doc(content, model)` |
...
| Academic papers, scientific PDFs with heavy LaTeX | **Nougat-base** |
...
├── resources/
│   ├── endpoints.yml             # 4 serving endpoints + 4 registered models
...
│   ├── log_models.py             # log all 4 models to UC + set Production alias
│   └── models/
│       ├── florence.py
│       ├── phi3_vision.py
│       ├── granite_vision.py
...
│   ├── 01_log_models.py
│   ├── 02_smoke_test.py
│   └── 04_eval.py                # mlflow.genai.evaluate() across all 4 endpoints
├── app/
│   ├── app.yaml
│   ├── streamlit_app.py
│   └── requirements.txt
...
for model in florence phi3_vision granite_vision nougat; do
...
# 5. Smoke-test all four endpoints.
...
| Nougat-base | CC-BY-NC 4.0 | [Meta AI](https://ai.meta.com) (Menlo Park, USA) — note: non-commercial license |
